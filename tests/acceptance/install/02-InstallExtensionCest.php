...
class InstallExtensionCest
{
	public function install(\AcceptanceTester $i)
	{
        ...
        $pathredCORE = $i->getConfiguration('package url' . 'extension/redCORE/tests/releases/redCORE.zip');
		...
		$pathEvent = $i->getConfiguration('packages url' . 'releases/redevent.zip');
