...
class RedeventAnalytics
{
	...
	{
		$options = $this->buildOptions($submit_key);
		RedformHelperAnalytics::recordTransMeasurementProtocol($submit_key, $options);
	}
	...
	{
		$attendeeRow = JTable::getInstance('RedEvent_register', '');
		$attendeeRow->load(array('submit_key' => $submit_key));

		$model = JModel::getInstance('Eventhelper', 'RedeventModel');
