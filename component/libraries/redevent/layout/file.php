 ...
 * @since  3.0
 */
class RedeventLayoutFile extends RLayoutFile
{
	/**
	...
	public function getDefaultIncludePaths()
	{
		...
		array_splice($paths, count($paths) - 2, 0, JPATH_LIBRARIES . '/redevent/layouts');
	...
	protected function refreshIncludePaths()
	{
		...
		{
			$redEventLayoutPath = JPATH_LIBRARIES . '/redevent/layouts';

			// If we already added the path, then do not add it again
			if ($this->includePaths[count($this->includePaths) - 2] != $redEventLayoutPath)
			{
				// Comes after (1 - lower priority) Frontend base layouts
				array_splice($this->includePaths, count($this->includePaths) - 2, 0, $redEventLayoutPath);
			}
