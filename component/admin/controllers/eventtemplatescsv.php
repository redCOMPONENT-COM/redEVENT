 ...
 * @since    3.0
 */
class RedeventControllerEventtemplatescsv extends RControllerForm
{
	/**
	...
	{
		$this->setRedirect('index.php?option=com_redevent&view=eventtemplatescsv');
	}
	...
	public function import()
	{
		...
		if (!isset($jformFiles['import']))
		{
			...
			$this->redirect('index.php?options=com_redevent&view=eventtemplatescsv', $msg, 'error');
		}
		...
		if (!$handle)
		{
			...
			$this->setRedirect('index.php?option=com_redevent&view=eventtemplatescsv', $msg, 'error');
		...
		if (!count($fields))
		{
			$msg .= "<p>Error parsing column names. Are you sure this is a proper csv export ?"
				. "<br />try to export first to get an example of formatting</p>\n";
			$this->setRedirect('index.php?option=com_redevent&view=eventtemplatescsv', $msg, 'error');
		...
		if (count($records))
		{
			$model = $this->getModel('eventtemplatescsvimport');
		...
		$this->setRedirect('index.php?option=com_redevent&view=eventtemplatescsv', $msg);
	}
}
