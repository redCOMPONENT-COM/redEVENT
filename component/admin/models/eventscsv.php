...
class RedeventModelEventscsv extends RModelAdmin
{
	...
	public function getItems()
	{
		...
		$query->select('e.id, e.title, e.alias')
			->select('e.summary, e.datdescription, t.details_layout, t.meta_description, t.meta_keywords')
			->select('e.datimage, e.published, e.registra, e.unregistra')
			->select('t.notify, t.notify_subject, t.notify_body, t.redform_id, t.juser')
			->select('t.notify_on_list_body, t.notify_off_list_body, t.notify_on_list_subject, t.notify_off_list_subject')
			->select('t.show_names, t.notify_confirm_subject, t.notify_confirm_body')
			->select('t.review_message, t.confirmation_message, t.activate, t.showfields')
			->select('t.submission_types, e.course_code, t.submission_type_email, t.submission_type_external')
			->select('t.submission_type_phone, t.submission_type_webform, t.show_submission_type_webform_formal_offer')
			->select('t.submission_type_webform_formal_offer, e.max_multi_signup, t.submission_type_formal_offer')
			->select('t.submission_type_formal_offer_subject, t.submission_type_formal_offer_body')
			->select('t.submission_type_email_body, t.submission_type_email_subject, t.submission_type_email_pdf')
			->select('t.submission_type_formal_offer_pdf, t.send_pdf_form, t.pdf_form_data, t.paymentaccepted')
			->select('t.paymentprocessing, t.enable_ical')
