...
class RedEventModelSettings extends JModel
{
	...
	function &getData()
	{
		$query = 'SELECT * FROM #__redevent_settings WHERE id = 1';
	...
	function checkin()
	{
		$item = & $this->getTable('redevent_settings', '');
		if(! $item->checkin(1)) {
	...
	function store($post)
	{
		...
		$settings 	= & JTable::getInstance('redevent_settings', '');
