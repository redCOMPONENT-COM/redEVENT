...
/**
 * View class for Bundles list
 *
 * @package  Redevent.admin
 * @since    3.2.0
 */
class RedeventViewBundles extends RedeventViewAdmin
{
	/**
	...
	{
		return JText::_('COM_REDEVENT_PAGETITLE_BUNDLES');
	}
	...
	public function getToolbar()
	{
		...
		{
			$new = RToolbarBuilder::createNewButton('bundle.add');
			$firstGroup->addButton($new);
		}

		if ($user->authorise('core.edit', 'com_redevent'))
		{
			$edit = RToolbarBuilder::createEditButton('bundle.edit');
			$secondGroup->addButton($edit);
		}

		if ($user->authorise('core.edit.state', 'com_redevent'))
		{
			$publish = RToolbarBuilder::createPublishButton('bundles.publish');
			$thirdGroup->addButton($publish);

			$unPublish = RToolbarBuilder::createUnpublishButton('bundles.unpublish');
			$thirdGroup->addButton($unPublish);
		}

		if ($user->authorise('core.delete', 'com_redevent'))
		{
			$delete = RToolbarBuilder::createDeleteButton('bundles.delete');
