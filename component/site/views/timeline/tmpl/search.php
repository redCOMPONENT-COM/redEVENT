...
<div id="redevent" class="timeline-search">
	<h2><?php echo JText::_('COM_REDEVENT_PAGETITLE_TIMELINE_SEARCH'); ?></h2>

	<form action="<?php echo JRoute::_(RedeventHelperRoute::getSearchTimelineRoute()); ?>" method="post" id="adminForm">
		<div class="filters">
			...
			<?php if ($this->customsfilters && count($this->customsfilters)): ?>
				...
				<?php endforeach; ?>
			<?php endif; ?>
			<button type="button" onclick="document.getElementById('filter').value='';document.getElementById('adminForm').submit();" class="btn-reset-filter-redvent"><?php echo '<span class="pink">X </span>'.JText::_('COM_REDEVENT_RESET' ); ?></button>
		</div>
	</form>
	...
	<div id="sessions">
		...
		<div class="session-detail">
			<div class="row">
				<div class="col-md-2 session-date">
					<?php
					$date = JFactory::getDate($session->dates);
					?>
					<span class="session-weekday"><?php echo $date->format('D'); ?></span>
					<span class="session-monthday"><?php echo $date->format('d/m'); ?></span>
				</div>
				<div class="col-md-2 session-venue">
					<?php
					echo $session->venue;
					?>
				</div>
				<div class="col-md-2 session-image">
					<?php
					if ($session->datimage)
					{
				...
				<div class="col-md-4 session-desc">
