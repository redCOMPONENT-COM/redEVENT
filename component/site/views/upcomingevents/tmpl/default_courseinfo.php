					...
					if ($event->prices && count($event->prices))
					{
						foreach ($event->prices as $p) 
						{
							if (empty($p->image)) {
								...
								$venues_html .= '<div class="vlink webform '.$p->alias.'">'.JHTML::_('link', JRoute::_(RedeventHelperRoute::getSignupRoute('webform', $event->id, $event->xref, $p->slug)), JHTML::_('image', $imagepath.$p->image,  JText::_($p->name), 'width="24px" height="24px"')).'</div> ';
							}
						}
					}
					else {
