				...
				<table>
					<?php if ($this->params->get('show_filter')) : ?>
						<tr>
							...
							<td>
								...
								<button
									onclick="document.getElementById('filter').value='';document.getElementById('adminForm').submit();"><?php echo JText::_('COM_REDEVENT_RESET'); ?></button>
							</td>
						</tr>
					<?php endif; ?>
					...
					<?php foreach ($this->customsfilters as $custom): ?>
						<tr>
							...
							<td>
								<?php echo $custom->renderFilter(array('class' => "inputbox dynfilter"), isset($filter_customs_values[$custom->id]) ? $filter_customs_values[$custom->id] : null); ?>
							</td>
						</tr>
					<?php endforeach; ?>
				</table>
