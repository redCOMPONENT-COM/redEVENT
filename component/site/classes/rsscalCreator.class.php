 ...
 */
  function rsscalCreator( $title=null, $link=null, $description=null, $version=null ) {
 ...
 * @return void
 */
  function rsscalElement( $elementName=null, $elementContent=null, $attributes=null ) {
  ...
  function createOutput( $newlinechar ) {
    ...
    if( !empty( $this->elementName ) && empty( $this->elementContent ))
      ...
      if( is_array( $this->elementContent )) {
        ...
        foreach( $this->elementContent as $cix => $elementContentPart ) { // new group, repeat name
          if( is_array( $elementContentPart )) {
            if(( 0 < $cix ) &&  !empty( $this->elementName ))
              $str .= '<'.$this->elementName.'>'.$newlinechar;
            foreach( $elementContentPart as $elementContentPart2 ) {     // new element within group
 ...
 * @uses function rsscalElement::rsscalElement
 */
  function rsscalChannel( $title, $link, $description, $resource=null ) {
 ...
 * @uses function rsscalElement::rsscalElement
 */
  function rsscalImage ( $title, $url, $link ) { // all required
 ...
 * @uses function rsscalElement::rsscalElement
 */
  function rsscalItem( $title, $link, $description=null ) {
 ...
 * @uses function rsscalElement::rsscalElement
 */
  function rsscalTextinput ( $title, $description, $name, $link ) {
 ...
 * @return void
 */
  function rsscalVersion ( $rsscalCreator ) {
 ...
 * @uses function rsscalElement::rsscalElement
 */
  function rsscalVersion1_0 ( $rsscalCreator ) {
    $this->version     =  $rsscalCreator->version;
    $this->content = new rsscalElement( 'rdf:RDF' );
    $this->content->attributes['xmlns:rdf'] = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#';
 ...
 * @uses function rsscalElement::rsscalElement
 */
  function rsscalVersion2_0 ( $rsscalCreator ) {
