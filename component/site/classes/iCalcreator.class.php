 ...
 */
	...
	{
		if (empty($value)) if ($this->getConfig('allowEmpty')) $value = null;
		else return FALSE;
...
class calendarComponent
{
	...
	{
		if (empty($week)) if ($this->getConfig('allowEmpty')) $week = null;
		else return FALSE;
	...
	{
		if (empty($exruleset)) if ($this->getConfig('allowEmpty')) $exruleset = null;
		else return FALSE;
	...
	{
		if (empty($statcode) || empty($text)) if ($this->getConfig('allowEmpty')) $statcode = $text = null;
		else return FALSE;
	...
	{
		if (empty($rruleset)) if ($this->getConfig('allowEmpty')) $rruleset = null;
		else return FALSE;
	...
	function setTrigger($year, $month = null, $day = null, $week = FALSE, $hour = FALSE, $min = FALSE, $sec = FALSE, $relatedStart = TRUE, $before = TRUE, $params = FALSE)
	{
		if (empty($year) && empty($month) && empty($day) && empty($week) && empty($hour) && empty($min) && empty($sec))
			if ($this->getConfig('allowEmpty'))
			{
				...
				return FALSE;
		if ($this->_isArrayTimestampDate($year))
		{ // timestamp
