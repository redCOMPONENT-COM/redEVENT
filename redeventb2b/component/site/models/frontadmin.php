...
class RedeventModelFrontadmin extends RedeventModelBaseeventlist
{
	...
	public function quickbook($user_id, $xref)
	{
		...
		if ($pricegroup->price)
		{
			$options['baseprice'] = $pricegroup->price;
		}
		...
		if ($details->notify)
		{
			$registrationmodel->sendNotificationEmail($submit_key);
		}

		$registrationmodel->notifyManagers($submit_key);
	...
	protected function getPricegroup($xref)
	{
		...
		$query->select('id, price, currency');
